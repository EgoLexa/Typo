...
def show():
	...
	now = time.time()
	d_int = lambda x: int(0.4 + (now - x)/86400)
	days = lambda x: 'day' if d_int(x)==1 else 'days'
	days_past = lambda t: 'today' if d_int(t)==0 else f'{d_int(t)} {days(t)} ago'
	...
	d_ = int((df.time.iloc[-1] - df.time.iloc[0])/86400) + 1
	days_ = 'day' if d_==1 else 'days'
	title = f'In {d_} {days_} you typed {df.shape[0]} text lines ({txt_mean} words per line)'
	if df.shape[0]>20:
